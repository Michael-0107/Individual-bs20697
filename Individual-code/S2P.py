...
class ShortSeq2PointDisaggregator(Disaggregator):
    ...
    def disaggregate(self, mains, output_datastore, meter_metadata, **load_kwargs):
        ...
        for chunk in mains.power_series(**load_kwargs):
            ...
            # appliance_power[appliance_power < 62] = 5
    ...
    #         self.mmax_x = np.array(ds)[0]
    #         self.mmin_y = np.array(hf.get('disaggregator-data').get('mmin_y'))[0]
    #         self.mmin = np.array(hf.get('disaggregator-data').get('mmin'))[0]
    #         self.mmax_y = np.array(hf.get('disaggregator-data').get('mmax_y'))[0]
    ...
    #         gr.create_dataset('mmin_y', data=[self.mmin_y])
    #         gr.create_dataset('mmin', data=[self.mmin])
    #         gr.create_dataset('mmax_y', data=[self.mmax_y])
    ...
    def _create_model(self):
        ...
        # 1D Conv
        model.add(tf.keras.layers.Conv1D(30, 10, activation='relu', input_shape=(self.window_size,1), padding="same", strides=1))
        # model.add(tf.keras.layers.Dropout(0.5))
        model.add(tf.keras.layers.Conv1D(30, 8, activation='relu', padding="same", strides=1))
        # model.add(tf.keras.layers.Dropout(0.5))
        model.add(tf.keras.layers.Conv1D(40, 6, activation='relu', padding="same", strides=1))
        # model.add(tf.keras.layers.Dropout(0.5))
        model.add(tf.keras.layers.Conv1D(50, 5, activation='relu', padding="same", strides=1))
        ...
        # opt = Adadelta(rho=1.0, epsilon=1e-09)
        ...
        print(model.summary())
        print('adadelta')
        # print(adam_optimizer.learning_rate)
